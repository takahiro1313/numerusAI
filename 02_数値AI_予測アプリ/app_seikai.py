...
演習用（app_ensyu.py）の編集セルを fit/predict 記入済みにしたもの。
そのまま「学習」「推論」を押せば動く。中身（裏方・画面）は演習用と同一で、
編集セルの初期値だけが異なる。
...
def _do_fit():
    ...
    save_all_predictions(st.session_state.model, load_data())
...
# 🟢 学習（fit）の編集セル初期値（正解・記入済み）
FIT_SNIPPET = '''# 🟢 ===== ここを編集 =====
model.fit(X_train, y_train)
# 🟢 ===== 編集はここまで =====
'''

# 🟢 推論（predict）の編集セル初期値（正解・記入済み）
